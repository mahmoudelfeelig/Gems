...
public final class SpyBackstab {
    ...
    public static void apply(ServerPlayerEntity attacker, LivingEntity target) {
        ...
        target.damage(attacker.getDamageSources().playerAttack(attacker), bonus);
        AbilityFeedback.burstAt(attacker.getServerWorld(), target.getPos().add(0.0D, 1.0D, 0.0D), ParticleTypes.CRIT, 10, 0.2D);
