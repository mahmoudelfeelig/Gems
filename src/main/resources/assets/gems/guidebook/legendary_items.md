...
- Legendary crafts take time to complete and may have per-item limits, depending on server balance.
...
- Each legendary item has up to 2 inscription slots by default (server-tunable).
- Slot counts, rarity tiers, roll weights, and magnitudes may vary by server.
...
- Universal mob blacklist: applies to Hypno Staff, Summoner summons, and Astra Soul Capture.
